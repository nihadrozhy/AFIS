...
import { fetchArrivalFlights, fetchArrivalFlightsOffline, fetchMoreArrivalFlights } from '../../redux/slice/arrivalsSlice';
...
const Arrivals: React.FC<Props> = ({ navigation, item }) => {
    const dispatch: AppDispatch = useDispatch();
    const { width } = useWindowDimensions();
    // const [state, setState] = useState<Flight[]>(FlightDetails);

    const { flights, loading, error } = useSelector((state: RootState) => state.arrivals);
    ...
    useEffect(() => {
        async function fetchData() {
          ...
          if(item.iata !== undefined){
            if (isConnected) {
              dispatch(fetchArrivalFlights(item.iata));
            } else {
                dispatch(fetchArrivalFlightsOffline(item.iata));
            }
          }
        }
    ...
    const loadMore = () => {
        if (item.iata !== undefined) {
            dispatch(fetchMoreArrivalFlights(item.iata, page));
        }
